from app.schema import Soulmate, TripPurpose
...
def calculate_soulmate_score(db, user1, user2):
	score = 0
	...
	user1_trip_purposes = db.query(TripPurposes).filter(TripPurposes.users.any(id=user1.id)).all()
	user2_trip_purposes = db.query(TripPurposes).filter(TripPurposes.users.any(id=user2.id)).all()
	matching_trip_purposes = len(set(user1_interests) & set(user2_interests))
	score += matching_trip_purposes

	user1_departures = db.query(Departures).filter(Departures.users.any(id=user1.id)).all()
	user2_departures = db.query(Departures).filter(Departures.users.any(id=user2.id)).all()

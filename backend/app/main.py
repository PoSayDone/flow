...
async def add_user(user: schema.UserBase, db: db_dependency):
    ...
    if db.query(models.Users).filter(models.Users.mail == user.mail).all():
        raise HTTPException(
            status_code=409, detail="User with this mail already exisits"
        )
    db.add(user)
    db.commit()
    return {"ok": True}
...
async def update_user(
    user: user_dependency, user_update: schema.UserUpdateBase, db: db_dependency
):
    if not user:
...
@app.delete("/departures/{location_id}")
...
@app.delete("/arrivals/{location_id}")
...
async def add_match(
    interested_in_user_id: UUID, user: user_dependency, db: db_dependency
):
    if not user:
        raise HTTPException(status_code=401, detail="Not authorized")
    match = models.Matches(
        id=uuid.uuid4(), user_id=user["id"], interested_in_user_id=interested_in_user_id
    )
    db.add(match)
    db.commit()
    return {"ok": True}
...
async def add_user_location(data: schema.PivotTableBase, db: db_dependency):
    data = models.UsersDepartures(user_id=data.user_id, location_id=data.id)
    db.add(data)
    db.commit()
    return {"ok": True}
...
async def delete_user_location(user_id: UUID, location_id: int, db: db_dependency):
    user_location = db.get(models.UsersDepartures, (user_id, location_id))
...
async def get_users_departures(db: db_dependency):
    result = db.query(models.UsersArrivals).all()
    if not result:
        raise HTTPException(status_code=404, detail="No departures found")
    return result
...
@app.get("/user_arrivals/")
async def get_user_departures(user: user_dependency, db: db_dependency):
    if not user:
        raise HTTPException(status_code=401, detail="Not authorized")
    result = (
        db.query(models.UsersArrivals)
